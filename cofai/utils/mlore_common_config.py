...
def get_head(p, backbone_channels, task):
    ...
    if task == 'scene':
        from cofai.backbone.mlore_transformers.heads import MLPHead
        return MLPHead(backbone_channels, p.TASKS.NUM_OUTPUT[task])
    if p['head'] == 'conv':
        from cofai.backbone.mlore_transformers.heads import ConvHead
        return ConvHead(backbone_channels, p.TASKS.NUM_OUTPUT[task])
    elif p['head'] == 'deconv':
        from cofai.backbone.mlore_transformers.heads import DEConvHead
...
def get_loss(p, task=None):
    ...
    if task == 'edge':
        ...
        from cofai.losses.loss_functions import CrossEntropyLoss
        criterion = CrossEntropyLoss(ignore_index=p.ignore_index)
    elif task == 'normals':
        from cofai.losses.loss_functions import L1Loss
...
def get_criterion(p):
    ...
    from cofai.losses.loss_schemes import MultiTaskLoss
