"""
MLoRE Backbone Wrapper for CoFAI Framework

This module wraps the RFC's MLoRE (Multi-task Low-Rank Expert) backbone 
to provide standard encode()/decode() interfaces compatible with the CoFAI framework.
...
class MLoREBackboneLarge(MLoREBackbone):
    ...
    def __init__(
        ...
        if stage == 'stage0':
            from cofai.backbone.mlore_transformers.MLoRE_baseline_nocompress import MLoRE_vit_large_patch16_384
            self.backbone = MLoRE_vit_large_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        elif stage == 'stage1':
            from cofai.backbone.mlore_transformers.MLoRE_coding_input_featcom import MLoRE_vit_large_patch16_384
            self.backbone = MLoRE_vit_large_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        elif stage == 'stage2':
            from cofai.backbone.mlore_transformers.MLoRE_coding_input_featcom_mona import MLoRE_vit_large_patch16_384
            self.backbone = MLoRE_vit_large_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        else:
