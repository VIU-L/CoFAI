"""
MLoRE Model for CoFAI Framework

This module implements MLoRE (Multi-task Low-Rank Expert) models following
the CoFAI framework design specification for FrameCodec and VideoCodec.
...
Framework specification: https://faymek.github.io/CoFAI/framework/
"""
...
class MLoREFrameCodec(CompressionModel):
    ...
    This FrameCodec implements the CoFAI framework specification
    ...
    def _init_backbone(self, p, stage, pretrained, img_size, drop_path_rate):
        ...
        if stage == 'stage0':
            from cofai.backbone.mlore_transformers.MLoRE_baseline_nocompress import MLoRE_vit_base_patch16_384
            self.backbone = MLoRE_vit_base_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        elif stage == 'stage1':
            from cofai.backbone.mlore_transformers.MLoRE_coding_input_featcom import MLoRE_vit_base_patch16_384
            self.backbone = MLoRE_vit_base_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        elif stage == 'stage2':
            from cofai.backbone.mlore_transformers.MLoRE_coding_input_featcom_mona import MLoRE_vit_base_patch16_384
            self.backbone = MLoRE_vit_base_patch16_384(
                p=p, pretrained=pretrained, drop_path_rate=drop_path_rate, img_size=img_size
            )
        else:
    ...
    def _init_heads(self, p):
        ...
        from cofai.heads.mlore_heads import create_mlore_heads
    ...
    def compress(self, x: torch.Tensor, tasks: List[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        Following the CoFAI DataUnitCodec interface, compresses input
    ...
    def decompress(
        ...
        **kwargs
        ...
        Following the CoFAI DataUnitCodec interface, decodes coded_unit
        to task outputs.
...
class MLoREVideoCodec(nn.Module):
    ...
    This VideoCodec implements the CoFAI framework specification
    ...
    def compress_video(
        ...
        Following the CoFAI VideoCodec interface, compresses all frames
        ...
        for idx, frame in enumerate(tqdm(video_reader, desc="Compressing")):
            ...
            if not isinstance(frame, torch.Tensor):
                ...
                if hasattr(frame, 'rgb'):
                    # Use rgb property if available (CoFAI video reader)
                    frame_rgb = frame.rgb
    ...
    def decompress_video(
        ...
        Following the CoFAI VideoCodec interface, decodes coded_data
...
class MLoREWrapperCodec(nn.Module):
    ...
    while conforming to CoFAI interfaces.
    ...
    def __init__(self, p, checkpoint_path=None):
        ...
        from cofai.backbone.MLoRE_wrapper import MLoREWrapper_coding
        from cofai.utils.mlore_common_config import get_backbone, get_head
