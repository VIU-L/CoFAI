"""
Dinov2 + VQFC wrapper for feature compression and classification evaluation.

This module provides a lightweight "codec-style" class (similar in spirit to
lamofc.py's Dinov2OrigSlideOnlyPatchCodec) but built on the VQFC model
defined in vqfc_model.py.

Typical usage:
    codec = Dinov2VQFCClsCodec(
        compressor_type="VQFC",
...
class Dinov2VQFCCodec(nn.Module):
    def __init__(
        self,
        vqfc_kwargs: Optional[Dict[str, Any]] = None,
        ...
        vqfc_kwargs = vqfc_kwargs or {}
        # if not hasattr(VQFC, "VQFC"):
        #     raise ImportError("Can't find VQFC from vqfc_model.py ")

        self.vqfc: nn.Module = VQFC(**vqfc_kwargs)
    ...
    def forward(
        ...
        """
        feat_hat, mse_loss, rd_loss, rate, encoding_inds = self.vqfc(feat, **kwargs)
    ...
    def compress(
        ...
        """
        if get_ready and hasattr(self.vqfc, "uncondi_entropy_model"):
            self.vqfc.uncondi_entropy_model.get_ready_for_compression()

        feat_hat, mse_loss, strings, encoding_inds = self.vqfc.compress(feat)
        coded_unit = {
    ...
    def decompress(
        self,
        coded_unit: Union[Dict[str, Any], List[bytes]],
        feat_shape: Union[Tuple[int, ...], torch.Size, None] = None,
    ) -> torch.Tensor:
        if isinstance(coded_unit, dict):
            strings_dict = coded_unit.get("strings", {})
            if "vqfc" not in strings_dict:
                raise ValueError("coded_unit['strings'] must contain 'vqfc'")
            strings = strings_dict["vqfc"][0]
            shape = coded_unit["pstate"]["feat_shape"]
        else:
            if feat_shape is None:
                raise ValueError("feat_shape is required when using legacy VQFC decompress inputs")
            strings = coded_unit
            shape = feat_shape

        feat_hat = self.vqfc.decompress(strings, tuple(shape))
        if not isinstance(feat_hat, torch.Tensor):
            raise TypeError(
                f"VQFC decompress output must be torch.Tensor, got {type(feat_hat).__name__}"
            )
        return feat_hat

    def state_dict_compressor(self) -> Dict[str, Any]:
        """save vqfc parameters only"""
        return self.vqfc.state_dict()

    def load_state_dict_compressor(
        self, state_dict: Dict[str, Any], strict: bool = True
    ) -> None:
        """load vqfc only"""
        self.vqfc.load_state_dict(state_dict, strict=strict)
    ...
    def seg_eval_from_feature_files(
        ...
        *,
        list_file: str = f"{PROJECT_ROOT}/examples/vqfc/cfg/val_100.txt",
        img_root: str = f"{PROJECT_ROOT}/data/VOC2012",
        feat_dir: str = f"{PROJECT_ROOT}/features/vqfc/seg/test",
        feat_aug_dir: str = f"{PROJECT_ROOT}/features/vqfc/seg/test",
        head_dataset: str = "voc2012",
        head_type: str = "linear",
        num_classes: int = 21,
        device: str = "cuda",
    ) -> Dict[str, Any]:
        """Segmentation mIoU eval using feature files (original vs VQFC recon).
        ...
        def create_segmenter(cfg, backbone_model):
            ...
            import cofai.backbone.dinov2.eval.segmentation.models  # noqa: F401
        ...
        cfg = mmcv.Config.fromfile(
            f"{PROJECT_ROOT}/examples/vqfc/cfg/dinov2_vitg14_{head_dataset}_{head_type}_config.py"
        )
